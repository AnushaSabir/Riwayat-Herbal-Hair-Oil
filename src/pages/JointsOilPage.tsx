  ...
  return (
    <main className="min-h-screen bg-background overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-1/4 left-0 w-96 h-96 bg-sky-500/5 blur-[150px] rounded-full" />
        <div className="absolute bottom-1/4 right-0 w-96 h-96 bg-cyan-500/5 blur-[150px] rounded-full" />
      </div>

      {/* ── Navbar ── */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-md border-b border-sky-500/10">
        <div className="container mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button
              onClick={() => navigate("/")}
              className="flex items-center gap-2 text-foreground/50 hover:text-sky-400 transition-colors text-sm group"
            >
          ...
          <div className="hidden md:flex items-center gap-8">
            {["Benefits", "Ingredients", "Results", "Videos"].map((item) => (
              <a key={item} href={`#${item.toLowerCase()}`}
                className="text-[11px] uppercase tracking-[0.3em] font-display text-foreground/60 hover:text-sky-400 transition-colors font-semibold">
                {item}
              </a>
            ))}
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 bg-sky-500/10 hover:bg-sky-500/20 border border-sky-500/30 text-sky-400 px-4 py-2.5 rounded-full text-xs font-display uppercase tracking-widest transition-all hover:scale-105"
          >
      ...
      <section className="relative min-h-screen flex items-center pt-20">
        ...
        <div className="absolute right-0 top-0 bottom-0 w-full lg:w-[48%] bg-sky-500/5 rounded-l-[200px] z-0 border-l border-sky-500/10" />
        ...
        <div className="container mx-auto px-4 lg:px-8 grid lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          >
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <span className="inline-flex items-center gap-2 text-sky-400 text-xs tracking-[0.4em] uppercase font-serif mb-3 border border-sky-500/30 px-4 py-1.5 rounded-full bg-sky-500/5">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {["Relieves Pain", "Reduces Swelling", "Improves Flexibility"].map((tag) => (
                <span key={tag} className="text-xs text-sky-400 border border-sky-500/30 px-3 py-1.5 rounded-full bg-sky-500/5 font-medium">
                  ✓ {tag}
                </span>
              ))}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-10">
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                onClick={handleAddToCart}
                className="px-8 py-4 bg-sky-500 text-white font-display text-base rounded-full uppercase tracking-widest font-semibold shadow-lg transition-all"
              >
                Order Now — Rs. 1,299
              </motion.button>
              <motion.button
                ...
                className="px-8 py-4 border-2 border-sky-500/40 text-sky-400 bg-transparent hover:bg-sky-500/10 font-display text-base rounded-full transition-all uppercase tracking-widest"
              >
                Learn More
              </motion.button>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex text-sky-400 text-sm">{Array(5).fill("★").join("")}</div>
          ...
          <div className="relative flex items-center justify-center min-h-[400px] sm:min-h-[500px] translate-x-0 sm:translate-x-4 md:translate-x-8 lg:translate-x-16 xl:translate-x-24 scale-[0.95] sm:scale-100 origin-center">
            ...
            <div className="flex flex-col gap-3 sm:gap-5 z-20 mx-0.5 sm:mx-1">
              {ingredients.slice(0, 4).map((_, i) => (
                ...
                >
                  <div className="w-3 sm:w-6 h-px border-t border-dashed border-sky-500/40" />
                  <svg width="7" height="7" viewBox="0 0 8 8" className="text-sky-400/50 flex-shrink-0"><path d="M0 4 L6 0 L6 8 Z" fill="currentColor" /></svg>
                </motion.div>
              ))}
            </div>
            ...
            >
              ...
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-24 sm:w-40 h-8 bg-sky-500/20 blur-2xl rounded-full" />
            </motion.div>
            ...
            <div className="flex flex-col gap-3 sm:gap-5 z-20 mx-0.5 sm:mx-1">
              {ingredients.slice(4).map((_, i) => (
                ...
                >
                  <svg width="7" height="7" viewBox="0 0 8 8" className="text-sky-400/50 flex-shrink-0"><path d="M8 4 L2 0 L2 8 Z" fill="currentColor" /></svg>
                  <div className="w-3 sm:w-6 h-px border-t border-dashed border-sky-500/40" />
                </motion.div>
              ))}
            </div>
      ...
      <section id="videos" className="relative py-24 bg-card/30">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="flex justify-center gap-3 mb-8 flex-wrap">
            {videos.map((v, i) => (
              <button key={i} onClick={() => setActiveVideo(i)}
                className={`px-5 py-2 rounded-full text-xs font-display uppercase tracking-widest transition-all ${activeVideo === i ? "bg-sky-500 text-white" : "border border-sky-500/30 text-sky-400/60 hover:border-sky-500/60"}`}>
                {v.label}
              </button>
            ))}
          </div>
          ...
          <div className="max-w-sm mx-auto">
            <div className="glass-card p-2 md:p-3 rounded-3xl relative group shadow-2xl border border-sky-500/20">
              ...
              <div className="absolute -bottom-4 -right-4 w-24 h-24 bg-sky-500/20 blur-[40px] rounded-full pointer-events-none" />
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {benefits.map((b, i) => (
              <motion.div
                ...
                className="group p-6 rounded-2xl border border-sky-500/10 bg-card/40 hover:border-sky-500/30 hover:bg-card/60 transition-all duration-300 hover:-translate-y-1"
              >
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 max-w-6xl mx-auto">
            {testimonials.map((t, i) => (
              <motion.div
                ...
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.1, duration: 0.6 }}
                className="p-6 rounded-2xl border border-sky-500/10 bg-card/40 flex flex-col gap-3"
              >
                <div className="flex text-sky-400 text-sm">{Array(t.rating).fill("★").join("")}</div>
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                onClick={handleAddToCart}
                className="px-10 py-4 bg-sky-500 text-white font-display font-bold text-base rounded-full uppercase tracking-widest shadow-lg"
              >
                Order Now — Rs. 1,299
              </motion.button>
              <motion.button
                ...
                whileTap={{ scale: 0.97 }}
                onClick={() => navigate("/")}
                className="px-10 py-4 border-2 border-sky-500/40 text-sky-400 rounded-full font-display font-semibold text-base uppercase tracking-widest hover:bg-sky-500/10 transition-all flex items-center gap-2 justify-center"
              >
